...
fun ServiceListScreen(
    ...
    Scaffold(
        topBar = {
            CustomAppBar(
                titleText = stringResource(R.string.app_bar_title_service_list),
        ...
        if (viewModel.isLoading) {
            ...
            if (viewModel.services.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(items = viewModel.services) {
                        CardListItem(title = it.date?.toDate()?.toFormattedString() ?: it.id, onClick = {
                            navController.navigate("${Screen.ServiceDetailsScreen.route}/${false}/${it.id}")
                        })
                    }
                }
            }
